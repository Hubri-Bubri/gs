<template>
  <div :style='styleObj' :draggable='isDraggable' @drag.stop='drag' @dragstart.stop='dragStart' @dragover.stop='dragOver'
  @dragenter.stop='dragEnter' @dragleave.stop='dragLeave' @drop.stop='drop' @dragend.stop='dragEnd' class='dnd-container'>
    <div :class='{"is-hover":isHover||subFulder(model)}' @click="toggle()"  @mouseover='mouseOver' @mouseout='mouseOut' @dblclick="changeType">
    <div :style="{ 'padding-left': (this.depth - 1) * 24 + 'px' }" :id='model.id' class='treeNodeText'>
        <slot :nodeName="model.name"  >
<!-- :isClicked='isClicked'  -->
<!--     <i :class="[(thrId== model.id || (parId == model.id)) ? 'fa fa-folder-open' : 'fa fa-folder']"
    style="font-size: 14px; color: #fd7e14; padding-left:10px;"></i> -->
    <i :class="[subFulder(model) ? 'fa fa-folder-open' : 'fa fa-folder']"
    style="font-size: 14px; color: #fd7e14; padding-left:10px;"></i>
    <span style="font-size: 14px;padding-left:10px;padding-right:10px;white-space: nowrap">{{model.name}}</span>
...
<!-- 
    ...
    <div class='treeMargin' v-show="subFulder(model)"  >
      <drag-node v-for="item2 in model.children" :allowDrag='allowDrag' :allowDrop='allowDrop'
      :depth='increaseDepth' :model="item2" :key='item2.id' :defaultText='defaultText' :disableDBClick='disableDBClick' :class='{"is-hover":subFulder(item2)}' >

<!--          <slot :nodeName="item2.name" >
       <span :class="[isClicked ? 'nodeClicked' : '','vue-drag-node-icon']"></span>
          <span class='text'>{{item2.name}}</span> -->

<span style="padding:0px;margin:0px;width:100%;white-space:nowrap;" @click="selfclick(item2.id);">
<!-- {{ subFulder(item2)}} -->
           <i :class="[subFulder(item2) ? 'fa fa-folder-open' : 'fa fa-folder']"
            style="font-size: 14px; color: #fd7e14; padding-left:10px;" ></i>
          <span style="font-size: 14px;padding-left:10px;padding-right:10px;white-space: nowrap">{{item2.name}}</span>
        <!-- </slot> -->
</span>
    </drag-node>
    </div>
...
export default {
  ...
  methods: {
    subFulder(val){
      var result = null
          if (this.thrId==val.id){
            result = true
            if(this.selfClickResult) {
              result=this.folder
            }
          } else{
            result = false
      ...
      return  (result||this.parId==val.id)
    }, 
    selfclick(val){
      this.selfClickResult=(val==this.itemId)
      this.subFulder(val)
      // var result = true
      ...
      // this.isClicked = this.folder

      this.itemId=val
      // return result
    },
    toggle() {
// console.log(this.thrId, this.model.id, this.subFulder(this.model))
       this.selfclick(this.model.id)
