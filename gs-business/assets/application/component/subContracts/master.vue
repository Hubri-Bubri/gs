          ...
          <container-body>
            ...
            >
              ...
              <template #cell(insurance_number)="it">
                <b-form-input  placeholder="Enter insurance name"
              ...
              <template #cell(date)="it">
                <b-form-input 
                  ...
                  :value="it.item.date" placeholder="Enter date" :id="'ofdate'+ it.item.id" size="sm"/>
              </template>
...
export default {
  ...
  computed: {
    ...
    fieldsTable() {
      ...
      {
        key: 'firma',
        label: this.$t('topMenu.subContractor'),
        sortable: true,
        variant: 'default'
      },
      {
        key: 'number',
        label: this.$t('topMenu.subcontracts'),
        sortable: true,
        variant: 'default'
      },
      {
  ...
  methods: {
    ...
    changeDate(type, value){
      ...
      this.getSubContracts(this.old_done+'|'+this.fd+'|'+this.td)
    },
    ...
    keywordChange(e){
      ...
      if ((done+'|'+this.fd+'|'+this.td)!=this.old_done+'|'+this.fd+'|'+this.td){
        this.op=[]
        this.items=[]
        this.getSubContracts(done+'|'+this.fd+'|'+this.td)
        this.old_done = done
      }
    },
  },
  mounted(){
    this.totalRows = this.items.length
      this.get_type_works()
      this.getSubContracts('1|0000-00-00|0000-00-00')
      this.$options.sockets.onmessage = (data) => (data.data=='getSubContracts') ? this.getSubContracts('1|0000-00-00|0000-00-00'): ''
