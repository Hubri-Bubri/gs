<template>
  <div>
         <b-form-checkbox style="width:100%" plain name="partx" class="withoutbox handleTitle"
            :class="{ 'table-dark':toggle.indexOf(part.parts.part_name +tableId ) >= 0 }"
            :value="part.parts.part_name+tableId" @change="toggleItem(part.parts.part_name+tableId)">
            <b-link v-b-toggle="'partx' + tableId" class="butMore" style="width:100%">
               <span class="when-opened">- </span>
               <span class="when-closed">+ {{total(part.parts.part_content) | thousandSeparator }} €</span>
            </b-link>
            <i class="fas fa-dot-circle" style="font-size:12px"></i> 
            <span contenteditable="true" @input="toModel($event, part.parts.part_name)" @click.prevent.self>{{part.parts.part_name}}</span>
            <slot name="tableDel"></slot>
         </b-form-checkbox>
         <b-collapse :id="'partx'+tableId">
            <div :class="{ 'table-dark': toggle.indexOf(part.parts.part_name +tableId ) >= 0 }">
               <div>
                  <b-form-row>
                     ...
                     <div class="custom-control-inline tpercent">
                        ...
                        class="ccbox" :class="classRow(part.parts, key)">
                     ...
                     <div class="tdone text-center ccbox">
                       <b-form-checkbox plain class="withoutPad" v-if="showAllworkers()"
                       @change="toggleAllWorkers($event)" :checked="checkedAllWorkers()" />
                     </div>
                     ...
                     <b-form-row style="height:20px;" @mouseover="content.upHere = true" @mouseleave="content.upHere = false" 
                       :class="{howerRow: content.upHere, '': !content.upHere}" :key="content.id" >
                        ...
                        <div class="tprice text-right" style="font-weight: normal;">
                           ...
                           <div v-else>
                              {{persent_to_str(subIndex, part.parts.checkbox_list.allSelected,
                                part.parts.checkbox_list.selected, part.parts.part_content, 'summ')}}
                            </div>
                        </div>
                        <div  class="tsumm text-right " style="font-weight: normal;" v-if="addtaxColapse==true">
                           <div :style="content.status=='yes' ? '' : 'color:grey'" 
                              ...
                              @click="alttax[subIndex] = alttax[subIndex] ? false : true">
                              ...
                              :title="persent_to_str(subIndex, part.parts.checkbox_list.allSelected,
                              part.parts.checkbox_list.selected, part.parts.part_content, '%') | thousandSeparator">
                                  {{persent_to_str(subIndex, part.parts.checkbox_list.allSelected,
                                    part.parts.checkbox_list.selected, part.parts.part_content, '%') | thousandSeparator}}
                              </div>
                           </div>
                        ...
                        <div v-else  class="tsumm text-right" style="font-weight: normal;" >
                           <div :style="content.status=='yes' ? '' : 'color:grey'">
                              ...
                              <div v-else :title="persent_to_str(subIndex, part.parts.checkbox_list.allSelected,
                              part.parts.checkbox_list.selected, part.parts.part_content, '%') | thousandSeparator">
                                  {{persent_to_str(subIndex, part.parts.checkbox_list.allSelected,
                                    part.parts.checkbox_list.selected, part.parts.part_content, '%') | thousandSeparator}}
                              </div>
                        ...
                        <div class="custom-control-inline tpercent">
                           <div  v-for="(select_from_allSelected, key) in part.parts.checkbox_list.allSelected" >
                              <div v-for="(opt, indexSelect) in part.parts.checkbox_list.flavours[key]" v-if="indexSelect==subIndex" class="ccbox"
                               :class="classRow(part.parts, key)" :style="(content.done!=false) ? 'color:green' : ''">
            ...
            <b-row class="ttotalsumm">
               ...
               General {{ total(part.parts.part_content) | thousandSeparator }}</b-col>
            </b-row>
            <span style="color:grey">
               <b-row class="ttotalsumm">
                  <b-col align-self="end" class="text-right" style="margin-right:22.6%">
                  Alternative {{ alttotal(part.parts.part_content) | thousandSeparator}}</b-col>
               </b-row>
            </span>
            <br>
         </b-collapse>
...
export default {
    ...
    methods: {
        ...
        selectionRows(){
          if(this.part.parts.part_content.length == this.color.length){
            this.part.parts.part_content.forEach((val, index)=>{
              this.selectRow(this.part.parts.part_name, index)  
            })
          } else{
            this.part.parts.part_content.forEach((val, index)=>{
                if(this.color[this.color.indexOf(this.part.parts.part_name+ '-' + index)]==undefined){
                this.selectRow(this.part.parts.part_name, index)
              }
            })
          }
        ...
        scheckStatus() {
            var obj
            for (obj in this.part.parts.checkbox_list.selected) {
                var countStr = 0
                var check = 0
                this.part.parts.part_content.forEach((val, index) => {
                    if (val.unit == '%') {
                    ...
                    if ((countStr + check) == this.part.parts.checkbox_list.flavours[obj].length) {
                        var objAls
                        for (objAls in this.part.parts.checkbox_list.allSelected) {
                            if (objAls == obj) {
                                var dis1
                                var uncheck = 0
                                ...
                                if (uncheck != 0) {
                                    this.$set(this.part.parts.checkbox_list.allSelected, objAls, true)
                                }
                            }
                        }
                    } else {
                        this.$set(this.part.parts.checkbox_list.allSelected, obj, false)
                    }
        ...
        subPartDel(subDel_id) {
            if (confirm("Are you sure?")) {
                    this.part.parts.part_content.splice(subDel_id, 1);
                    for (var property in this.part.parts.checkbox_list.flavours) {
                        this.part.parts.checkbox_list.selected[property].splice(valuePart.parts.checkbox_list.selected[property]
                          .indexOf(this.part.parts.checkbox_list.flavours[property][subDel_id]), 1)
                        this.part.parts.checkbox_list.flavours[property].splice(subDel_id, 1)
                    }
                }
        },
        ...
        changeWorker(event, index){
          if(event==false){
              this.part.parts.part_content[index].done=false
          } else{
            this.part.parts.part_content[index].done=this.selectedWorkers
          }
        },
        toggleAllWorkers(event){
          if(event==false){
              this.part.parts.part_content.forEach(function(val){
                val.done=false
              })
          } else{
            this.part.parts.part_content.forEach((val)=>{
      ...
      checkedAllWorkers(){
        var countDone = 0
          ...
          if(countDone == this.part.parts.part_content.length || this.selectedWorkers==0){
            return true
          } else{
            return false
          }
    ...
    watch: {
        part: {
            handler: function(newVal) {
                ...
                for (var obj in this.part.parts.part_content) {
                    var tname = 't' + this.tableId + 'x' + obj
                    if (this.part.parts.part_content[obj].description_work.indexOf(' ') != -1) {
                                ...
                                if (val.search(/[-+*)(/]/gim) > -1) {
                                    if (val.search(/[0-9]/gim) > -1) {
                                        if (/[\d)]/.test(val[val.length - 1])) {
                                            ...
                                            indexReplace = this.part.parts.part_content[obj].description_work.indexOf(val) + indexRuslt + 6
                                            this.$refs[tname][0].quill.clipboard.dangerouslyPasteHTML(
                                            this.part.parts.part_content[obj].description_work.replace(val, sval))
    ...
    created() {
        this.tableId = Math.floor(Math.random() * (9999))
    }
}
</script>