...
<table class="table table-hover tableProject">
  <thead>
    <tr>
      <th v-for="(fild, index) in fields">{{fild.label}}&nbsp;&nbsp;&nbsp;</th>
    </tr>
  </thead>

    <draggable  :element="'tbody'" :options="{handle:'.handle',  group:'a', animation:150}"
                  :no-transition-on-drag="true" @end="checkMove" >
    <tr v-for="(tr, index) in items">
      <td class="handle text-center" @click="rowSelected(tr)" :style="(tr._rowVariant=='success')?rowColor:''" > {{ index + 1 }} </td>
      <td ><b-input size="sm" :style="(tr._rowVariant=='success')?rowColor:''" class="cForm-input" :value="tr.pos_num" @change="updateDate($event, 'pos_num', tr.id)"></b-input></td>
      <!-- <td ><b-input size="sm" style="min-width:150px;" :style="(tr._rowVariant=='success')?rowColor:''" class="cForm-input" :value="tr.name" @change="updateDate($event, 'name', tr.id)"></b-input></td> -->

 
      <td><div :style="(tr._rowVariant=='success')?rowColor:''+';width:100%;min-width:150px;padding:5px;'" :contenteditable="!disablefild('priceName', tr.id)"
        @focus="changeDisable('f', 'priceName', tr.id)" @blur="updateDate($event.target.innerText, 'name', tr.id);changeDisable('b', 'priceName', tr.id);"
        
        :id="'priceName'+tr.id" >{{tr.name}}</div></td>
      ...
      <td ><div :style="(tr._rowVariant=='success')?rowColor:''+';width:100%;min-width:150px;padding:5px;'" :contenteditable="!disablefild('priceDesc', tr.id)"
        @focus="changeDisable('f', 'priceDesc', tr.id)"  @blur="updateDate($event.target.innerText, 'desc', tr.id);changeDisable('b', 'priceDesc', tr.id)"

        :id="'priceDesc'+tr.id" >{{tr.desc}}</div></td>
      ...
      <td ><b-input size="sm" :style="(tr._rowVariant=='success')?rowColor:''" class="cForm-input" :value="tr.unit" @change="updateDate($event, 'unit', tr.id)"></b-input></td>
      <td ><b-input size="sm" :style="(tr._rowVariant=='success')?rowColor:''" class="cForm-input" :value="tr.price" @change="updateDate($event, 'price', tr.id)"></b-input></td>
      <td ><b-input size="sm" :style="(tr._rowVariant=='success')?rowColor:''" class="cForm-input" :value="tr.without" @change="updateDate($event, 'without', tr.id)"></b-input></td>
      <td ><b-input size="sm" :style="(tr._rowVariant=='success')?rowColor:''" class="cForm-input" :value="tr.percent" @change="updateDate($event, 'percent', tr.id)"></b-input></td>
      <td :style="(tr._rowVariant=='success')?rowColor:''" class="text-center"><b-link @click="delRow(tr.id)" class="fas fa-trash fa-w-16 text-center" /></td>
    </tr>
</draggable>
</table>
...
.t_index { width: 1%; text-align: center; }
.t_pos_num { width: 8%; }
.t_name { width: 20% !important; }
.t_desc { width: 52%; }
.t_unit { width: 5%; }
.t_price { width: 8%; }
.t_without { width: 3%; }
