<template>
  <div>
    <template v-for='(item,index) in newData'>
      <drag-node :idNode='idNode' :model='item' :allowDrag='allowDrag' :allowDrop='allowDrop' :depth='increaseDepth'
      :defaultText='defaultText' :disableDBClick='disableDBClick' :key='index' v-slot='slotProps'>
        <slot :nodeName="slotProps.nodeName" :isClicked='slotProps.isClicked' :idNode='idNode'></slot>
      </drag-node>
    </template>
  </div>
</template>
