...
@routes.get('/domage_images_company')
async def method (request):
    return web.json_response(await Projects.get_damage_images_company(request.query['id']))
...
async def method (request):
    return web.json_response(await Projects.get_damage_images_sub(request.query['id']))

@routes.get('/domage_images_sperson')
async def method (request):
    return web.json_response(await Projects.get_damage_images_sperson(request.query['id']))
...
@routes.get('/updateProject')
async def method (request):
    try:
        result = web.json_response(await Project.update(request.query['id'], request.query['date'], request.query['fild']))
    except KeyError:
        try:
            return web.json_response('')
        except KeyError:
...
@routes.get('/balance')
async def method (request):
    return web.json_response(await Balance.select_balances())

@routes.get('/add_payment')
async def method (request):
    result = web.json_response(await Balance.add_payment(request.query['id'], request.query['value'], request.query['type']))
    for client in ws_clients:
...
async def method (request):
    ...
    for client in ws_clients:
        await client.send_str('getCustomerContact')
    return result
...
async def method (request):
    ...
    for client in ws_clients:
        await client.send_str('getSubContact')
    return result
  ...
  #      print(len(chunk))
   ...
   # print('4')
    return web.json_response(await Docs.upload_doc(
        file, filename, number, number_of_pages, group, added, user, page_content))
