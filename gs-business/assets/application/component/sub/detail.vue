      ...
      <b-card-group deck>
        <b-card no-body class="gs-container" :style="heightComponentforSm">
          <b-tabs card v-model="tabIndex"  class="tabs">
            <b-tab>
              <template #title>
                <span v-if="name==undefined">{{$t('subDetail.subContractor')}}</span>
                <span v-else :title="$t('subDetail.subContractor')+' '+toName(name)">{{toName(name)}}</span>
              </template>
              <container>
                <container-body>
                  <b-container fluid ref="hsub">
                    <b-row>
  ...
  methods: {
    toModelImag(enterVal){
        function findLevel(obj, id) {
            obj.forEach((val)=>{
                if (val.id==id){
                    axios.get('/update_name_images_sub_menu', {
                        params: {
                            name: enterVal,
                            id: id
                        }
    ...
    toModelImagPerson(enterVal){
        function findLevel(obj, id) {
            obj.forEach((val)=>{
                if (val.id==id){
                    axios.get('/update_name_images_person_menu', {
                        params: {
                            name: enterVal,
                            id: id
                        }
  ...
  this.$refs.moveImag2.show()
      ...
      removeImagPerson(){
       if (this.idNodeImgPerson==null){
            ...
            if (confirm(this.$t('alert.remove'))) {
                axios.get('/remove_image_person_menu', {
                    params: {
      ...
      removeImag(){
       if (this.idNodeImg==null){
            ...
            if (confirm(this.$t('alert.remove'))) {
                axios.get('/remove_image_sub_menu', {
                    params: {
    ...
    addImagPerson(){
      ...
      function findLevel(obj, id, project) {
        if (id==null){
            ...
            obj.forEach((val)=>{
                if (val.id==id){
                    axios.get('/add_images_person_menu', {
                        params: {
                            parent_id: val.id,
                            project: project,
                            newName: newName
