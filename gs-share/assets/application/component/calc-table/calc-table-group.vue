...
<!--     <b-modal size="sm" centered id="worker" ref="worker" title="Select Workers" body-class="workerHeight">
    ...
    <b-modal size="sm" centered id="ids" ref="ids" title="Select Ids" body-class="workerHeight">
            <b-form-checkbox-group buttons  button-variant="light" style="width:100%" v-model="selectedId" stacked :options="id_list"/>
            <template slot="modal-footer">
              <button type="button" left class="btn btn-secondary" @click="okInvoice">OK</button>
           </template>
        </b-modal>
    <b-modal size="sm" centered id="ids_sub" ref="ids_sub" title="Enter Ids" body-class="workerHeight">
            <b-input  v-model="selectedId" />
            <template slot="modal-footer">
      ...
      <b-col lg="6" md="12">
        <!--             <b-button class="customButton" @click="move">
          ...
          <b-button  class="customButton col-12 col-lg-1" v-b-modal.addPart :disabled="type=='Invoices'" style="min-width:55px;">
              Add Part
          </b-button>
          <b-button  class="customButton col-12 col-lg-1" @click="getSubCustomers()" style="min-width:55px;"  v-if="type=='Orders'">
              Add SUB
          </b-button>
<!--           <b-button  class="customButton col-12 col-lg-1" @click="addInvoice('Damage Description')"  style="min-width:55px;" v-if="(type=='Orders')"> 
              To Damage
          </b-button> -->
          <b-button  class="customButton col-12 col-lg-1"  @click="reports" :disabled="type=='Invoices'"  style="min-width:55px;">
              Reports
          </b-button>
          <b-button class="customButton col-12 col-lg-1" @click="worker" :disabled="type=='Invoices'" style="min-width:55px;">
            Workers
          </b-button>

          <b-button  class="customButton col-12 col-lg-1" @click="addInvoice('SInvoices')"  style="min-width:55px;" v-if="(type=='SUB')">
              SUB Invoice
          </b-button>

          <b-button  class="customButton col-12 col-lg-1" @click="addInvoice('Invoices')" style="min-width:55px;"  v-if="(type=='Orders')">
              To Invoice
          </b-button>
...
<b-col class="cForm col-12 col-lg-4" style="padding:0px;" slot="Type" v-if="((type=='Orders') || (type=='Offers'))">
  Type:&nbsp;
  <b-select class="customButton col-11"  :value="type" @change="offerChangeType(id, $event)" :options="getOpt()"  />
</b-col>

<b-col class="cForm col-12 col-lg-4" style="padding:0px;" slot="Work">
  Work:&nbsp;
  <b-select class="customButton col-11"  :value="tmp.work" :options="works" @change="updateItem($event, 'work', tmp.id)" />
</b-col>
         ...
         <div v-for="(part, index) in value" :key="part.id">
          ...
          <calc-table :nowTableId="value" :value="sort(value[index])" :addtaxColapse="addtaxColapse" :tableId="index" :butDiscPerc="butDiscPerc" :unitType="unitType" :looks="looks"
            :color="color" :selectedWorkers="selectedWorkers" :toggle="value[index].parts.toggle" :type="type" :funcStop="funcStop"  v-on:changeSum="discOfPercent()"  >
               ...
               <b-checkbox style="" :disabled="type=='Invoices'" plain name="partx" slot-scope="table" class="withoutbox"
                    ...
                    <b-link v-b-toggle="'partx' + table.tableId" class="butMore" style="width:100%">
                       ...
                       <span class="when-closed">+ {{total(value[index].parts.part_content) | thousandSeparator }} €</span>
                    </b-link>
                    ...
                    <b-link :disabled="type=='Invoices'" class="fas fa-trash  butMore" style="padding-left: 0px; font-size:12px;" @click="partDel(value[index].parts.id)"/>
                </b-checkbox>
            </calc-table>
       ...
       return opts
        ...
        if(val=='SInvoices'){
          ...
          axios.get('/detect_invoice').then(response => {
            ...
            this.selectedId = response.data[(response.data.length-1)]
            this.$refs.ids.show()
          });
        }}
