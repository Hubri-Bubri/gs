        ...
        async with database.query() as Q:
            ...
            await (Q()
                     ...
                     .insert({
                       ...
                       T.items.number : result['number']+' '+str(result['id']),
            ...
            invoiceIdx = ( await (Q()
                ...
                .where(T.items.number == (result['number']+' '+str(result['id'])))
                .selectall())
             )
            invoiceId = invoiceIdx[len(invoiceIdx)-1]
            ...
            if type=='Invoices':
                ...
                await (Q()
                        ...
                        .where(T.items.number == (result['number']+' '+str(result['id'])))
                        .update({
                            T.items.number: result['number']+' '+str(numberInvoice['id'])+' '+str(result['id'])
                        }))
            if type=='SUB':
                await (Q()
                        .tables(T.items)
...
class Balance:
    ...
    async def select_balances(cls):
        async with database.query() as Q:
            ...
            # sub_result = ( await (Q()
            #     .tables(T.sub_items & T.type_for_table)
            #     .on(T.sub_items.type == T.type_for_table.id)
            #     .fields(
            #         T.sub_items.id,
            #         T.sub_items.number,
            #         T.sub_items.date,
            ...
            #         T.sub_items.comment
            #     )
            #     .where(T.type_for_table.type == 'Invoices')
            #     .selectall()))
            # dataFormProject = (await (Q(T.project)
            #                  .fields(
            #                      T.project.tax,
            #                      T.project.taxDub,
            #                      T.project.taxP,
            ...
            #                      T.project.disc,
            #                      T.project.discP,
            #                      T.project.butDiscPerc,
            #                      T.project.addtaxColapse
            #                 ).where(T.project.id == id)
            #                  .selectone()))

            # tax=(dataFormProject['tax'].replace(',', '.'))
            # taxDub=float(dataFormProject['taxDub'].replace(',', '.'))
            # taxP=float(dataFormProject['taxP'].replace(',', '.'))
            # taxPDub=(dataFormProject['taxPDub'].replace(',', '.'))
            # disc=float(dataFormProject['disc'].replace(',', '.'))
            # discP=float(dataFormProject['discP'].replace(',', '.'))
            # butDiscPerc=dataFormProject['butDiscPerc']
            # addtaxColapse=dataFormProject['addtaxColapse']
