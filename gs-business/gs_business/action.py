...
async def method (request):
    try:
        return web.json_response(await Project.update(request.query['id'], request.query['date'], request.query['fild']))
    except KeyError:
        try:
            return web.json_response('')
        except KeyError:
    ...
    #     updateProjectSend = 'project_detail_f'
...
async def method (request):
    result = web.json_response(await Invoice.add_invoice(request.query['id'], request.query['type'], request.query['number']))
    for client in ws_clients:
        await client.send_str('get_types_for_tables_f')
    return result
...
async def method (request):
        ...
        html = await Docs.select_doc(data['html']) # это id
