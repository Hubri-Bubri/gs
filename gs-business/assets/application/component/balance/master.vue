...
<b-row v-show="op" style="border-top: 1px solid black;padding-top:7px;">
      ...
      <b-col sm="7" class="cuestomRow">
        <b-row>
          ...
          <b-col cols="1"><b-link @click="op=false">X</b-link></b-col>
        </b-row>
        <b-row>
          <b-col cols="3" class="cForm">    
            Outstanding balance:
    ...
    countDigitals(val){
        ...
        getBalanceinit(){
          axios.get('/balance').then(response => {
            this.itemsinit =  response.data.reverse()
          });
          setTimeout(() => { 
            this.getBalance()
          }, 2000);
                            ...
                            if (v.type=='Invoices'){
                                if (withOut >= 99){
                                    ...
                                    if (v.number.split(' ').length==5){
                                       v.cnumber = v.number
                                      v.number = v.number.split(' ')[0]+' - '+v.number.split(' ')[3]
                                    }
                                    this.items.push(v)
                                }
                           }
...
balance(){
    ...
    setTimeout(() => {
        ...
        this.$options.sockets.onmessage = (data) => (data.data=='getBalance') ? this.getBalanceinit(): ''
  },1000);
        }
}
</script>