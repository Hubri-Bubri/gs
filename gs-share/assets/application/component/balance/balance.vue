  ...
  <b-container fluid>
      <b-table  stacked="lg" @row-clicked="$emit('dataToFoot', $event)" class="tableProject" striped hover  :items="items_search()" :fields="fieldsTable"  busy.sync="isBusy">
        ...
        <template slot="cnumber" slot-scope="it">
          <b-link :to="'/project/detail/'+it.item.project_id"  v-if="it.item.number.split(' ').length==3">
            {{
              countDigitals(it.item.number.split(' ')[0])
            }}
          </b-link>
          <b-link :to="'/project/detail/'+it.item.project_id"  v-if="it.item.number.split(' ').length==5">
            {{
              it.item.number.split(' ')[2]
            }}
          </b-link>
        </template>
 ...
 items_search() {
      ...
      if (this.keyword){
         return this.items.filter(item =>{
            var numberInv = this.countDigitals(item.number.split(' ')[1])+'-'+item.number.split(' ')[0].split('-')[1]
            var numberSubInv = item.number.split(' ')[1]
            var numberContr = this.countDigitals(item.number.split(' ')[0])
