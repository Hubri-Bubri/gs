<template>
    <container class="container-fluid">
        <container-header>
            <top-menu>                            <b-input-group><b-input placeholder="search" v-model="keyword" />
                            <template #append>
           ...
           <!-- <v-infinite-scroll :loading="loading" @bottom="nextPage" :offset='20' style="max-height: 80vh; overflow-y: scroll;"> -->
                        <b-table stacked="lg" hover :items="items_search" :fields="fields" @row-clicked="inItemClick"  class="tableProject" small busy.sync="isBusy">
                               <template slot="id" slot-scope="data">
                                  {{rpeNumber(data.item.project_number)}}-{{data.item.project_number_year}}
                                </template>
  ...
  <b-modal id="invoice" size="lg" title="List" ok-only  centered>
    <div v-for="name in types_for_tables">
                                   <span v-if="getItems(name.type).length!=0">{{name.type}}</span>
                                    <b-table class="tableProject" striped hover :items="getItems(name.type)" :fields="fieldsTable" v-if="getItems(name.type).length!=0">
                                ...
                                <span  class="text-content"  style="white-space: nowrap;">{{it.item.netto|thousandSeparator}}  &nbsp;</span>
                                      ...
                                      <template slot="status_set" slot-scope="it" >
                                          ...
                                          <b-form inline v-if="name.type=='Orders'">
                                            ...
                                            <b-form inline v-if="name.type=='Offers'">
                                                <b-form-select v-model="it.item.status_set" @change="updateItem($event, name.type, it.item.id)"
                                                class="w-100" :style="computeStyleOfferM(it.item.status_set)+';min-width:100px;'">
                                                  <option v-for="opt in order_options" :style="computeStyleOfferM(opt.text)">
                                                  {{opt.text}}
...
items_search() {
      ...
      if (this.keyword){
         return this.items.filter(item =>{
          ...
          if (item.edate!=null){
            if (item.edate.includes(this.keyword)){
              this.fields.edate.variant = 'info'
              return true
            }
          }
